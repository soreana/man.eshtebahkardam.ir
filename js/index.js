...
var love = setInterval(function() {
    ...
    var t_size = Math.floor(Math.random() * 65) + 10;
	var qoutes  = ["Pardon", "Apology", "Regretful","Sorry","Penitent","Shame"];
    var index_1 = Math.floor(Math.random() * 6) ;
    var index_2 = Math.floor(Math.random() * 6) ;
	

    $('.bg_heart').append("<div class='heart' style='width:" + r_size + "px;height:" + r_size + "px;left:" + r_left + "%;background:rgba(255," + (r_bg - 25) + "," + r_bg + ",1);-webkit-animation:love " + r_time + "s ease;-moz-animation:love " + r_time + "s ease;-ms-animation:love " + r_time + "s ease;animation:love " + r_time + "s ease'><p class='sorry' style='font-size: "+ (r_size)+"px;width:" + (r_size - 10) + "px;height:" + (10*r_size - 10) + "px;left:" + (r_left + r_num) + "%;color:rgba(255," + (r_bg - 25) + "," + r_bg + ",1)'>" + qoutes[index_1]+"</p></div>");


    $('.bg_heart').append("<div class='heart' style='width:" + (r_size - 10) + "px;height:" + (r_size - 10) + "px;left:" + (r_left + r_num) + "%;background:rgba(255," + (r_bg - 25) + "," + (r_bg + 25) + ",1);-webkit-animation:love " + (r_time + 5) + "s ease;-moz-animation:love " + (r_time + 5) + "s ease;-ms-animation:love " + (r_time + 5) + "s ease;animation:love " + (r_time + 5) + "s ease'><p class='sorry' style='font-size: "+ (t_size) +"px;width:" + (r_size - 10) + "px;height:" + (10*r_size - 10) + "px;left:" + (r_left + r_num) + "%;color:rgba(255," + (r_bg - 25) + "," + r_bg + ",1)'>"+ qoutes[index_2] +"</p></div>");
